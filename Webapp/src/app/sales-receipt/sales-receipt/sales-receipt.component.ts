...
export class SalesReceiptComponent implements OnInit {
    ...
    async SaveOrUpdatePurchase() {
        ...
        if (res.Type == "S") {            
            ...
            this.router.navigateByUrl(`/`, {skipLocationChange: true}).then(
                () => this.router.navigate([`/SalesReceipt/${res.AddtionalData}`])
            );
        }
        else {            
    ...
    Cancel() {
        this.helper.redirectTo('/SalesReceiptList');
    }
...
const routes: Routes = [
    { path: "", component: SalesReceiptComponent }
];
