...
@Component({
  ...
  styleUrl: './sales-receipt-list.component.css',
  providers: [ConfirmationService]
})
export class SalesReceiptListComponent {
  ...
  constructor(
    ...
    async GetStockEntryList()
    {
      ...
      if(res?.length > 0)
      {
          this.PurchaseReceiptList = res.filter(o=> o?.stock_entry_type == "SalesReceipt");
      }
      else
      {
    ...
    AddOrEditPurchase(purchase_id : any) {
        this.helper.redirectTo(`SalesReceipt/${purchase_id}`);
    }
...
const routes: Routes = [
  { path: "", component: SalesReceiptListComponent }
];
...
export class SalesReceiptListRoutingModule { }

@NgModule({
  declarations: [SalesReceiptListComponent],
  imports: [
      CommonModule,
      SalesReceiptListRoutingModule,
      ModuleData,
      TableModule,
      InputTextModule,
      ButtonModule,
...
export class SalesReceiptListModule { }
