...
@Component({
  selector: 'app-purchase-issue-list',  
  templateUrl: './purchase-issue-list.component.html',
  styleUrl: './purchase-issue-list.component.css',
  providers: [ConfirmationService]
})
export class PurchaseIssueListComponent implements OnInit{
  ...
  datePickerLocalOptions: any = {
    ...
    async GetStockEntryList()
    {
      this.helper.ShowSpinner();
        ...
        if(res?.length > 0)
        {
          this.PurchaseReceiptList = res.filter(o=> o?.stock_entry_type == "PurchaseIssue");
        }
        else
        {
    ...
    AddOrEditPurchase(purchase_id : any) {
        this.helper.redirectTo(`PurchaseIssue/${purchase_id}`);
    }
...
const routes: Routes = [
  { path: "", component: PurchaseIssueListComponent }
];
...
export class PurchaseIssueListRoutingModule { }

@NgModule({
  declarations: [PurchaseIssueListComponent],
  imports: [
      CommonModule,
      PurchaseIssueListRoutingModule,
      ModuleData,
      TableModule,
      InputTextModule,
      ButtonModule,
...
export class PurchaseIssueListModule { }
