...
export class PurchaseReceiptComponent implements OnInit {
    purchaseReceiptForm = this.fb.group({
        ...
        stock_entry_type: ['PurchaseReceipt'],
    ...
    async SaveOrUpdatePurchase() {
        ...
        if (res.Type == "S") {            
            ...
            this.router.navigateByUrl(`/`, {skipLocationChange: true}).then(
                () => this.router.navigate([`/PurchaseReceipt/${res.AddtionalData}`])
            );
        }
        else {            
    ...
    Cancel() {
        this.helper.redirectTo('/PurchaseReceiptList');
    }
...
const routes: Routes = [
    { path: "", component: PurchaseReceiptComponent }
];
