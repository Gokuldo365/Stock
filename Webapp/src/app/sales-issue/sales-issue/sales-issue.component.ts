...
@Component({
    selector: 'app-sales-issue',
    templateUrl: './sales-issue.component.html',
    providers: [ConfirmationService]
})

export class SalesIssueComponent implements OnInit {
    ...
    async SaveOrUpdatePurchase() {
        ...
        if (res.Type == "S") {            
            ...
            this.router.navigateByUrl(`/`, {skipLocationChange: true}).then(
                () => this.router.navigate([`/SalesIssue/${res.AddtionalData}`])
            );
        }
        else {            
    ...
    Cancel() {
        this.helper.redirectTo('/SalesIssueList');
    }
...
const routes: Routes = [
    { path: "", component: SalesIssueComponent }
];
...
export class SalesIssueRoutingModule { }

@NgModule({
    declarations: [SalesIssueComponent],
    imports: [
        CommonModule,
        SalesIssueRoutingModule,
        ModuleData,
        DropdownModule,
        TableModule,
        ButtonModule,
